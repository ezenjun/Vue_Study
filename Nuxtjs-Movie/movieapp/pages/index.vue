			...
			meta: [
				{
					hid:'description',
					name: 'description',
					content:'Get all the latest straming movies in theaters & online!',
				},
				{
					hid:'keywords',
					name: 'keywords',
	...
	methods:{
		async getMovies(){
			const data = axios.get(
				`https://api.themoviedb.org/3/movie/now_playing?api_key=716e013f8cb7dae99e2978360c4658c4&language=en-US&page=1`
			);
		...
		async searchMovies(){
			const data = axios.get(
				`https://api.themoviedb.org/3/search/movie?api_key=716e013f8cb7dae99e2978360c4658c4&language=en-US&page=1&query=${this.searchInput}`
			)
			const result = await data;
			result.data.results.forEach((movie) => {
